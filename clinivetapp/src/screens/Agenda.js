...
export default class Agenda extends Component {
  ...
  addTask = async task => {
    try {
      await axios.post(`${server}/consultas`, {
        desc: task.desc,
  ...
  deleteTasks = async id => {
    try {
      await axios.delete(`${server}/consultas/${id}`)
  ...
  toggleTask = async id => {
    try {
      await axios.put(`${server}/consultas/${id}/toggle`)
      await this.loadTasks()
    } catch (err) {
      showError(err)
    }
  ...
  loadTasks = async () => {
    try {
      ...
      const res = await axios.get(`${server}/consultas?date=${maxDate}`)
  ...
  render() {
    ...
    return (
      <View style={styles.container}>
        <AddTak isVisible={this.state.showAddTask}
          onSave={this.addTask}
